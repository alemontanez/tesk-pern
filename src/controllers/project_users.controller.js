...
import { createMember, updateMember } from '../services/project_users.service.js'
...
export const updateMembership = async (req, res) => {
  ...
  try {
    ...
    console.log(error)
    if (error.message === 'Membership not found' || error.message === 'Role not found' || error.message === 'User not found') {
      return res.status(404).json({ message: error.message })
    }
    return res.status(500).json({ message: 'Internal error' })
  }
